...
type
  TDataModule1 = class(TDataModule)
    ...
    procedure MoveDown;
    procedure MoveUp;
    procedure AddMovie(title:string;length:string;thumbnailPath:string; ondisk:boolean; trailerUrl: string);
  private
...
procedure TDatamodule1.AddMovie(title: string; length: string; thumbnailPath: string; ondisk: Boolean; trailerUrl: string);
begin
  try
  TitleTB.Insert;
  TitleTB['Title'] := title;
  TitleTB['Length'] := length;
  TitleTB['Thumbnail'] := thumbnailpath;
  TitleTB['OnDisk'] := ondisk;
  TitleTB['TrailerURL'] := trailerurl;
  TitleTB.post;
  PrintConsole('Movie added', 'info');
  except
    messagedlg('Error adding movie', mterror, [mbok], 0);
    printConsole('Movie could not be added', 'warn');
  end;
end;
